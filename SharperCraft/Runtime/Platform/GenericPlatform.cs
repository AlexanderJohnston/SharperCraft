...
using Serilog;
using SharperCraft.Runtime.Host;
using System;
...
namespace SharperCraft.Runtime.Platform
{
    ...
    {
        public GenericHost Host = new GenericHost();
        ...
        private INativeClass Create([Required] Assembly assembly, [Required] string @namespace, [Required] string className)
        {
            if (this.InvalidNamespace(assembly, @namespace))
                this.Crash($"[Generic Platform]: Namespace could not be located: {@namespace} in assembly: {assembly.FullName}");
